...
| Night-sky drawing       | `src/components/Sketch.astro`, generated by `docs/scene/scene_gen.py` (Kazbek traced from a photo, `kazbek-trace.json`) then `docs/scene/build_sketch.py` (adds the star canvas) |
| Friend's sketch          | `src/components/Doodle.astro` (source `docs/scene/doodle.svg`), in the footer |
